...
每次推送到 GitHub（包括只推送会议纪要等文档）都必须同步更新 [`docs/当前进度与协作交接.md`](docs/当前进度与协作交接.md)，让没有参与本轮工作的队友能直接知道：目前实际完成了什么、证据和限制是什么、下一步由谁认领、具体输入/输出/验收标准是什么，以及任务间如何衔接。若本次推送不改变实验状态，也要明确写明“实验进度未变化”，并保留仍有效的下一步任务。历史过程继续记在 [`docs/进度日志.md`](docs/进度日志.md)。
