...
- 跨物种、跨数据集的一致性验证；
- 冻结 DNA 预训练模型嵌入与 motif/间距特征的互补性分析（扩展路线）；
- Motif 与启动子强度、组合共现的分析（时间和标签允许时开展）。
...
DNA 大模型嵌入不替代 MEME/FIMO，而是作为并行的表示增强路线：先用冻结的 DNABERT-2 或 Nucleotide Transformer 提取序列向量，再检查这些向量是否能补充 motif、位置和间距特征。来源边界、实验设计和失败降级规则见 [`docs/05_DNA大模型嵌入扩展方案.md`](docs/05_DNA大模型嵌入扩展方案.md)。
